...
- **종목 정보 (`tickers`)**: 수집 대상 종목 리스트 관리. (완료)
- **시계열 데이터 (`market_data`)**: 주가 데이터(OHLCV) 저장 테이블. (완료)
- **거시경제 지표 (`macro_economic`)**: 금리, GDP 등 저장 테이블. (완료)
- **에이전트 로그 (`agent_logs`)**: 수집 성공/실패 여부를 기록하여 모니터링. (완료)
...
1. **자동화 스케줄러 구축 (`api/scheduler.py`)** (우선순위 높음)
   - 매일 정해진 시간에 `market.py`와 `fred.py`를 자동으로 실행하도록 설정해야 합니다.
   - 현재는 수동 실행만 가능한 상태입니다.

2. **기술적 분석 모듈 개발 (`api/analyzer.py`)**
   - 수집된 `market_data`를 읽어와 RSI, 이동평균선 등을 계산하는 로직이 필요합니다.
   
3. **AI 에이전트 초기 구현 (`api/agent.py`)**
   - OpenAI API를 연동하여, 분석된 데이터를 바탕으로 간단한 코멘트("매수 추천" 등)를 생성하는 기능을 만듭니다.
